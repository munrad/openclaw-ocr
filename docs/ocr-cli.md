...
Для child tasks, созданных через coordinator/runtime surface, `set-status` должен
нести matching `run_id` root orchestration. Если `run_id` отсутствует или не
совпадает, OCR намеренно не подтянет этот snapshot в tracker/root projection и
посчитает это integration issue в `get-orchestration` / `orchestration-health`.

Если передан неизвестный `state`, команда завершится с `arg_error`.
...
### `status-hygiene [--agents csv] [--apply true]`

Operator utility для поиска и, при явном `--apply true`, удаления suspicious
legacy mirrored fields из `openclaw:agents:status:*`.

```bash
ocr status-hygiene
# {"ok":true,"dry_run":true,"affected_agents":1,"suspicious_fields_total":3,"agents":[...]}

ocr status-hygiene --agents coder,tester --apply true
# {"ok":true,"dry_run":false,"removed_fields_total":3,...}
```

Инструмент не переписывает canonical поля статуса; он удаляет только очевидный
legacy мусор вроде mirrored key/value artifacts.
...
results, child agent statuses, and child status issue diagnostics.

```bash
ocr get-orchestration --task-id parser-hotfix
# {"ok":true,"task_id":"parser-hotfix","orchestration":{"status":"active","phase":"fanout_completed","children":[...],"child_result_summary":{"done":1,"failed":0,"pending":1,"unknown":0},"child_status_summary":{"completed":1,"queued":1},"child_status_issue_summary":{"run_id_mismatch":1,"missing_run_id":0}}}
```
...
# {"ok":true,"status":"active","count":2,"orchestrations":[{"task_id":"...","done_children":1,"pending_children":1,"failed_children":0,"run_id_mismatch_children":1,"integration_error_count":1,...}]}
```

### `orchestration-health [--status active|completed|failed|all] [--limit N]`

Aggregated operator metrics for root orchestrations and task-status trackers:

- active roots vs configured limit
- forced closes
- child `run_id` mismatches / missing `run_id`
- degraded tracker delivery (`pending`, `unconfirmed`, `suppressed`)
- `bootstrap_pending` trackers and retry history

```bash
ocr orchestration-health --status all --limit 20
# {"ok":true,"metrics":{"active_orchestrations":1,"forced_closed_orchestrations":1,"child_run_id_mismatches":2,"trackers_delivery_not_confirmed":1,...},"problematic_orchestrations":[...],"degraded_trackers":[...]}
```
